...
class Einkaufsliste:
    ...
    def save_as_pdf(self, einkaufsladen, target_folder):
        ...
        PDF.save_in_one_file(target_folder, "Gesamt_Einkaufsliste.pdf")

    def create_einkaufsliste(self,tage, laden):
        zutaten = {}
        for tag in tage:
            for gerichte in tag.gericht:
                for zutat in gerichte.zutat:
                    if zutat.lieferant == laden:
                        key = (zutat.artikelname, zutat.einheit, zutat.kategorie)
    ...
    def run(self, file_base_path, filename, workbooks, läden, output):
        self._excel = Excel()
        ...
        _einkaufsladen:Laden = []
        for laden in läden:
            zutaten = self.create_einkaufsliste(_tage, laden)
            zutaten = sorted(zutaten.values(), key=lambda z: z["kategorie"] or "")
            _einkaufsladen.append(Laden(name=laden, zutaten=zutaten))

        self.save_as_pdf(_einkaufsladen, target_folder=output)
